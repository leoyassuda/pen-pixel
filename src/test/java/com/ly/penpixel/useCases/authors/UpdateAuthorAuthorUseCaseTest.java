...
class UpdateAuthorAuthorUseCaseTest {
  @Mock
  ...
  void execute() {
    ...
    when(authorDAO.update(any(AuthorDataModel.class))).thenReturn(1);

    Author result = updateAuthorUseCase.execute(author);
    verify(authorDAO, times(1)).update(authorDataModel);
